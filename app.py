...
def index():
    if request.method == "POST":
        ...
        contents = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[
        {"role": "user", "content": f"제목:{subject} '날씨:{weather},기분:{mood},사건:{content}' 의 내용으로 200자 이내로 일기를 적어줘"},
        ])
        print(contents)
        return render_template("result.html", contents=contents.choices[0].message.content)
